...
import { 
  ...
  ShoppingCart, Zap, Shield, AlertTriangle 
} from "lucide-react";
...
const Reports = () => {
  ...
  // FIX 2: Added more robust loading and error states
  if (loading) return (
  ...
  if (!data || !data.metrics) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4 text-slate-400">
      ...
      <p className="text-[10px] font-black uppercase tracking-[0.4em]">Intelligence Feed Offline</p>
    </div>
  );

  // Safely derive status data
  const statusData = [
  ...
  return (
    <div className="space-y-16 animate-in fade-in duration-700 pb-10 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-slate-100 border-b border-slate-100 pb-12">
        <Metric label="Settled Capital" value={`₹${(data.metrics.completedRevenue || 0).toLocaleString()}`} trend="+12.4%" positive={true} />
        <Metric label="Deficit (Rejected)" value={`₹${(data.metrics.lostRevenue || 0).toLocaleString()}`} trend="-2.1%" positive={false} />
        <Metric label="Market Potential" value={`₹${(data.metrics.totalPotential || 0).toLocaleString()}`} trend="Gross" positive={true} />
      </div>
              ...
              <div key={i} className="flex items-center justify-between group border-b border-slate-50 pb-4">
                ...
                <div className="flex items-center gap-6">
                  <div className="h-1.5 bg-slate-100 w-32 rounded-full overflow-hidden">
                    <div className="h-full bg-slate-900 group-hover:bg-amber-500 transition-all duration-500" style={{ width: `${(product.count / (data.topProducts[0]?.count || 1)) * 100}%` }}></div>
                  </div>
